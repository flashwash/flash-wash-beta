...
function getPathFromState(state, options = {}) {
  ...
  while (current) {
    ...
    let currentParams = _objectSpread({}, route.params);
    ...
    while (route.name in currentOptions && hasNext) {
      ...
      if (route.params) {
        ...
        currentParams = Object.fromEntries(Object.entries(route.params).map(([key, value]) => [key, (stringify === null || stringify === void 0 ? void 0 : stringify[key]) ? stringify[key](value) : String(value)]));
    ...
    if (currentOptions[route.name] !== undefined) {
      path += pattern.split('/').map(p => {
        const name = p.replace(/^:/, '').replace(/\?$/, ''); // If the path has a pattern for a param, put the param in the path

        if (p.startsWith(':')) {
          const value = allParams[name]; // Remove the used value from the params object since we'll use the rest for query string
    ...
    if (route.state) {
      ...
      const query = _queryString.default.stringify(currentParams);
...
function createNormalizedConfigs(options, pattern) {
  return Object.fromEntries(Object.entries(options).map(([name, c]) => {
