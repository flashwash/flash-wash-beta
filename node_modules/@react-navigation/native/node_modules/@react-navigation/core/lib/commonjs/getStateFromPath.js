...
function createNormalizedConfigs(screen, routeConfig, routeNames = [], initials, parentPattern) {
  const configs = [];
...
function createConfigItem(screen, routeNames, pattern, path, parse) {
  ...
  const regex = pattern ? new RegExp("^(".concat(pattern.split('/').map(it => {
    ...
    return "".concat((0, _escapeStringRegexp.default)(it), "\\/");
  }).join(''), ")")) : undefined;
  return {
    screen,
    regex,
